...
4. **Gap Detection:**
   The `GapDetector` analyzes consecutive nodes in the attack graph. If the logical "distance" between two events exceeds a dynamically computed threshold (based on temporal delays and continuity breaks), a **Gap** is identified.
...
6. **Evidence Verification:**
   The `EvidenceVerifier` performs a final sanity check on the highest-scoring candidate. It asserts six dimensions (`temporal`, `host`, `user`, `process`, `technique`, `contradiction`) to ensure the hypothesis doesn't violently contradict known facts.
   ...
   - **UNKNOWN:** A gap existed, but the available evidence was insufficient or ambiguous (e.g., multiple candidates had tied evidence scores). CyberScope aggressively withholds classification rather than hallucinating an answer.
